...
## Loading
The selected model starts loading when the hotkey goes down, so it loads while you speak. It then stays in memory between takes: Whisper in `whisper_cache.rs`, ONNX models in `model_slot.rs`. Idle unload (Settings) and auto-pause free either one. Switching or removing a model frees the old ONNX model at once.
## Long takes
